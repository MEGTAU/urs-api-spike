...
class Controller {
    ...
    static getRequestBodyName(request) {
        if (request.openapi.schema.requestBody && request.openapi.schema.requestBody['x-codegen-request-body-name']) {
            return request.openapi.schema.requestBody['x-codegen-request-body-name'];
        }
        throw new Error("x-codegen-request-body-name' not provided in Open API schema");
        // Could use a default value if not specified
    }
    ...
    static async handleRequest(request, response, serviceOperation) {
        try {
            let collectRequestParams = this.collectRequestParams(request);
            const serviceResponse = await serviceOperation(collectRequestParams);
